...
> 投稿：`1 セーブ画面でボタンを2回押すと落ちます 2 タイトルの「はじめる」が「はじめめる」になってます`

| ID | 種別 | 内容 | 発見者 | 報告日 | 発生Ver | ステータス |
|---|---|---|---|---|---|---|
| B-01 | バグ | セーブ画面でボタンを2回押すと落ちます | （Discordの表示名） | 2026-08-06 | ver0.00.02 | 未対応 |
| B-02 | 誤字 | タイトルの「はじめる」が「はじめめる」になってます | （Discordの表示名） | 2026-08-06 | ver0.00.02 | 未対応 |

**1つの投稿に「1 …／2 …／3 …」と複数書かれていれば、項目ごとに別の行になります。**
番号を振って何件でも書いてもらえます。

**`内容` は要約ではなく、原文の抜き書きです。** その項目に当たる部分を、語を変えずにそのまま写します。
要約させると、意味が静かに反転することがあったためです
（`クレジットに名前を表記してほしい` → `クレジット表記をやめてほしい`）。
とくに依頼や要望は、**何をどうしてほしいのかが述語に乗っている**ので、縮めると方向が消えます。

言い換えを機械的に弾く仕組みは**入れていません**。
「原文の部分文字列か」で判定すると、助詞が違う程度で大半が引っかかり、
⚠️ ばかりのシートになって読む人の負担にしかならなかったためです。
抜き書きはプロンプト側で徹底し、**取りこぼしは人が直す**方針にしています。

`CATEGORIES` の**末尾の語が受け皿**になります（語彙に当てはまらないときここに寄せます）。
`その他` のような語を必ず最後に置いてください。

種別の語を並べるだけだと、モデルは語感で判断します。短い投稿が近そうな語に落ちるのを防ぐには、
`CATEGORY_HINTS` で各語の意味を渡してください（`バグ:動作がおかしい,タスク:これからやる作業` の形）。
**語彙の全部に付けるのがおすすめです。** 一部だけ付けると、意味を書いた語のほうに寄ります。

### 種別が本文から決まらないなら、聞かない

`CATEGORIES` を**1語だけ**にすると、**モデルに種別を聞きません。** その語で全行が埋まります。

集めている場所によっては、種別が本文から決まりません。実際に踏んだ例です。
...
上2つは名詞だけで、不具合ともやることとも読めます。3つ目は1文の中に不具合とやることが同居しています。
**書いた本人しか知らない情報**なので、語彙をどう設計しても当たりません。

無理に選ばせると、当たっているように見える間違いが混ざります。**語彙の中で間違えた場合は
`種別補完` の印すら付きません**（語彙外のときしか付かないため）。1語にして人が仕分けるほうが速くて正確です。
...
```
gh variable set TOPIC      --body 'ゲームのテストプレイで見つかった不具合・誤字・要望と、これからやる作業'
gh variable set CATEGORIES --body 'バグ,誤字,バランス,要望,文書,タスク,その他'
gh variable set CATEGORY_HINTS --body 'バグ:動作がおかしい・落ちる,誤字:文字や表記の誤り,バランス:難易度や配分の話,要望:こうしてほしいという提案,文書:説明書やreadmeの話,タスク:これからやる作業・予定,その他:上のどれでもない'
...
gh variable set ID_PREFIX  --body '要望:R,タスク:T,*:B'
...
| `CATEGORIES` | `progress,done,issue,other` | 種別の語彙。**末尾が受け皿**。**1語だけにすると種別を聞かなくなる** |
| `CATEGORY_HINTS` | （なし） | 種別の語の意味。`バグ:動作がおかしい,…` の形。**語彙の全部に付ける** |
...
| 内容が言い換えられている | `原文` 列と見比べて手で直してください。とくに依頼は方向（〜してほしい／やめてほしい）を確認 |
| 種別が的外れになる | `CATEGORY_HINTS` で各語の意味を渡してください。語を並べるだけだと語感で判断されます |
| リンクを押すと「起動できませんでした」 | GASエディタで `checkGitHub` を実行。トークンと承認を検査します |

### 間違った行を直すとき

Botは言い換えを機械的に弾きません。おかしい行は手で直します。**直した行がBotに壊されることはありません。**

1. **`メッセージID` 列で行を特定してください。** 画面上の位置で数えないこと。
   スプレッドシートは編集を確定した拍子にスクロールするので、**隣の行を上書きしやすい**です
2. `内容` と `原文` を並べて見ます。とくに依頼は**方向**（〜してほしい／やめてほしい）が要注意です
3. `内容` を直すときは、`原文` の該当箇所をそのまま写します（要約しない）

| | |
|---|---|
| 直した行が複製されないか | されません。Bot は同じメッセージIDの行を二度作りません |
| 行ごと消したら | 復活しません。カーソルが既に通り過ぎています |
| `ID` を直していいか | **触らないでください。** 種別を手で変えると `ID_PREFIX` とずれます（`タスク:T` なのに `B-xx` のまま） |
| ID を揃えたい | standalone 版の `reorderTab_` を実行します。ただし**全体を振り直す**ので、対応済の行を参照している箇所が動きます。`備考` が空でない行は振り直されません |
